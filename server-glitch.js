...
app.get('/', (req, res) => {
  res.send(`
    <h1>FPS Game WebSocket Server</h1>
    <p>WebSocket server is running. This server handles the multiplayer functionality for the FPS game.</p>
    <p>Active connections: ${Object.keys(io.sockets.sockets).length}</p>
    <p>Active games: ${Object.keys(activeGames).length}</p>
  `);
});
